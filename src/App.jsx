  ...
  if (!user) {
    return (
      <div style={{ padding: '2rem', textAlign: 'center' }}>
        <h1>SightCall 2.0</h1>
        <input type="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} style={{ padding: '8px', margin: '5px', width: '200px' }} />
        <br />
        <input type="password" placeholder="Senha" value={password} onChange={(e) => setPassword(e.target.value)} style={{ padding: '8px', margin: '5px', width: '200px' }} />
        <br />
        <button onClick={handleAuth} style={{ padding: '10px 20px', marginTop: '10px' }}>
          {isLogin ? 'Entrar' : 'Cadastrar'}
        </button>
        <br />
        <a onClick={() => setIsLogin(!isLogin)} style={{ color: 'blue', cursor: 'pointer', display: 'block', marginTop: '10px' }}>
